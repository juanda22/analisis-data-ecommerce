...
Antes tenía este mismo análisis armado en Google Sheets con Apps Script (podés verlo en [analisis-datos-bidcom](../analisis-datos-bidcom)), pero lo pasé a Python porque quería practicar pandas con un caso más real, más allá de los ejercicios de la facultad.
...
1. Carga los CSV (o genera datos de prueba si no encuentra los reales, así se puede probar sin tener el dataset a mano)
2. Normaliza canales, métodos de pago y canal de adquisición
3. Valida que los IDs referenciados existan en su tabla correspondiente
4. Corrige las cantidades según las reglas de arriba
5. Recalcula `first_order_date` a partir de las órdenes reales de cada cliente
6. Arma una comparación entre un escenario "ideal" (todo lo que se cargó) y uno "real" (solo lo que efectivamente se vendió), para ver cuánto se pierde por cancelaciones/devoluciones
7. Calcula margen por categoría y por canal, y clientes nuevos vs. recurrentes por mes
